    ...
    <div className="space-y-3">
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          ...
          <select value={userId} onChange={(e) => setUserId(e.target.value)} className="w-full rounded-lg border border-line bg-ink-700 px-3 py-2 text-sm text-body">
        ...
        <div>
          ...
          <input type="number" value={delta} onChange={(e) => setDelta(Number(e.target.value))} className="w-full rounded-lg border border-line bg-ink-700 px-3 py-2 text-sm text-body tabular" />
        </div>
      </div>
      <div>
        <label className="mb-1 block text-xs text-muted">Reason (required)</label>
        <input value={reason} onChange={(e) => setReason(e.target.value)} placeholder="e.g. Correcting duplicate signup grant" className="w-full rounded-lg border border-line bg-ink-700 px-3 py-2 text-sm text-body" />
      </div>
