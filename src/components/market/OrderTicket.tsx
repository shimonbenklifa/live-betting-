...
export function OrderTicket({ market }: { market: MarketVM }) {
  ...
  return (
    <div className="rounded-xl border border-line bg-ink-800 p-4">
      <div className="mb-3 text-sm font-semibold text-white">Order ticket</div>
      ...
      <select
        ...
        className="mb-3 w-full rounded-lg border border-line bg-ink-700 px-3 py-2 text-sm text-white"
      >
      ...
      <div className="mb-3 flex items-center gap-2">
        <input
          ...
          className="w-full rounded-lg border border-line bg-ink-700 px-3 py-2 text-sm text-white tabular"
        />
        {[10, 25, 100].map((n) => (
          <button key={n} onClick={() => setShares(n)} className="rounded-md bg-ink-600 px-2 py-2 text-xs text-gray-200 hover:bg-ink-500">
            {n}
          </button>
        ))}
      </div>
...
function Row({ label, value, strong }: { label: string; value: string; strong?: boolean }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <span className={cn("tabular", strong ? "font-semibold text-white" : "text-gray-200")}>{value}</span>
    </div>
  );
}
